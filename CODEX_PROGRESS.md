...
완료된 이천베베로드 기능을 비공개 웹페이지와 GitHub에 반영한다.
...
1. 변경 파일과 웹 자산만 스테이징한다. 기존 `.site-deploy/`와 원본 `resources/pic/new/`는 사용자 제공 원본이므로 커밋에 포함하지 않는다.
2. 기능 단위 커밋을 만든다.
3. Sites에 새 버전을 배포하고 성공 URL을 확인한다.
4. GitHub `main`에 푸시한다.

## 다음 세션 시작 위치

배포가 아직 완료되지 않았다면 `CODEX_PROGRESS.md`의 다음 작업을 따라 `a88bf28` 이후 변경 파일을 커밋하고, `.openai/hosting.json`의 기존 프로젝트에 새 버전을 배포한다.
